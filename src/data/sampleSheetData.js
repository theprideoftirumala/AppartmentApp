...
export const GUIDE_ROWS = [
  ['Two workbooks', 'The Pride of Tirumala-APP is the old books (Nov 2020–Aug 2026), including the Summary tab with surplus/deficit as typed there. The Pride of Tirumala-LIVE is the Sep 2026+ cash book. The website never creates TPT-MaintenanceTracker.', 'Open old books in the app at #/old. Founding owner: Settings → Backups → Create live books. After that, type Sep 2026+ on the LIVE file (or in the app).'],
  ...
  ['Tab: Payees', 'Watchman and vendor pay details. Add the UPI ID here, then use Pay in the app (GPay / PhonePe / any UPI app). Same UPI ID, or same name and phone, is a duplicate.', 'No UPI IDs were in the old workbook. Do not invent a UPI ID. Phones for lift, electrician, plumber, and garbage came from the Notes tab.'],
  ['Tab: Live Summary', 'Only on The Pride of Tirumala-LIVE. Same shape as the old Summary: flats across months, then expense labels (Cleaning, Generator, Lift, Service charges, R&M, …). Collection and expense cells are formulas from Maintenance and Expenses.', 'Do not type collection or expense amounts here — that duplicates. To add a month, copy the last month column. Monthly surplus (deficit) = collection − expenses. Running available balance = opening (old Summary green cell at create) + each month surplus. Service charges is shown for the old layout; those amounts are logged as Repairs & Maintenance so they appear on R&M only once.'],
