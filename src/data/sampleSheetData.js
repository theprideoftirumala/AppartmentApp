...
export const GUIDE_ROWS = [
  ...
  ['Tab: Payees', 'Watchman and vendor pay details. GPay and PhonePe use a 10-digit phone. A UPI ID is optional and overrides the phone. Same phone or same UPI ID is a duplicate.', 'Do not invent a UPI ID. Phones for lift, electrician, plumber, and garbage came from the Notes tab.'],
  ['Tab: Live Summary', 'Only on The Pride of Tirumala-LIVE. Same shape as the old Summary: flats across months, then expense labels (Cleaning, Generator, Lift, Service charges, R&M, …). Collection and expense cells are formulas from Maintenance and Expenses. Starts with Aug-26 only.', 'Do not type collection or expense amounts here — that duplicates. Add the next month in the app (Maintenance → Add next month). Do not pre-fill a year of empty columns. Opening B2 already includes August as of the old Summary green cell — do not re-type August if it is in that cell. Monthly surplus (deficit) = collection − expenses. Running available balance = opening + each month surplus.'],
