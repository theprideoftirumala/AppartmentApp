...
import { Shield, LogIn, Building2, Lock, BarChart3, AlertCircle } from 'lucide-react';
import { useAuth } from '../contexts/AuthContext';
import { APP_NAME } from '../config/constants';

export default function Login() {
  const { signIn, loading: authLoading, error: authError, accessDenied } = useAuth();
  ...
  const handleSignIn = async () => {
    ...
    try {
      ...
      if (err.message?.startsWith('ACCESS_DENIED')) {
        // Detailed error set in AuthContext — do not overwrite
        setError(null);
      } else {
        ...
        {isAccessDenied && (
          <div className="login-access-denied">
            ...
            <div>
              ...
              <p>Contact the Treasurer or President of your apartment to request access.</p>
            </div>
          </div>
        )}
