...
**Dashboard** reads the active workbook (LIVE if bound, otherwise APP). Edit Maintenance/Expenses on that file, then refresh. Cards show those typed amounts when Live Summary formulas still disagree (banner lists the mismatch). Do not type amounts on Live Summary. Maintenance month/flat (and Expense month) are stored as text. Live Summary is recreated when A4 is not `tpt-live-v5`; collection cells are `SUMIFS` + `TO_TEXT($A)`. Unused `Sheet1` on LIVE is removed. Never `C$5`. Add next month fills the first gap after Aug-26. Still Due (K) and Monthly Summary are rewritten in the same pass. Do not mint a second LIVE file.
