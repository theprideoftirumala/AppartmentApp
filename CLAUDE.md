...
**Dashboard** reads the active workbook (LIVE if bound, otherwise APP). Edit Maintenance/Expenses on that file, then refresh. Cards show those typed amounts when Live Summary formulas still disagree (banner lists the mismatch). Do not type amounts on Live Summary. Live Summary `SUMIFS` use the month as text (`"Aug-26"`), never `C$5`. Missing `tpt-live-v2` in A4 forces a rewrite. Add next month fills the first gap after Aug-26. Still Due (K) and Monthly Summary are rewritten in the same pass.
